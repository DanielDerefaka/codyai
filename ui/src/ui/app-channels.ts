...
export async function handleWhatsAppStart(host: CodyAIApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: CodyAIApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: CodyAIApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: CodyAIApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: CodyAIApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: CodyAIApp): string {
...
function resolveGatewayHttpAuthHeader(host: CodyAIApp): string | null {
...
function buildGatewayHttpHeaders(host: CodyAIApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: CodyAIApp) {
...
export function handleNostrProfileToggleAdvanced(host: CodyAIApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: CodyAIApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: CodyAIApp) {
  const state = host.nostrProfileFormState;
