...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds CODYAI_GATEWAY_TOKEN and should be reinstalled.",
  ...
  issues.push({
    ...
    message:
      "Gateway service CODYAI_GATEWAY_TOKEN does not match gateway.auth.token in openclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.CODYAI_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.CODYAI_GATEWAY_TOKEN?.trim() || undefined;
}
