...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CODYAI_GATEWAY_PORT).toBe("18789");
    expect(env.CODYAI_GATEWAY_TOKEN).toBeUndefined();
    expect(env.CODYAI_SERVICE_MARKER).toBe("codyai");
    expect(env.CODYAI_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CODYAI_SERVICE_VERSION).toBe("string");
    expect(env.CODYAI_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    expect(env.CODYAI_WINDOWS_TASK_NAME).toBe("CodyAI Gateway");
    if (process.platform === "darwin") {
      expect(env.CODYAI_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", CODYAI_PROFILE: "work" },
      port: 18789,
    });
    expect(env.CODYAI_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    expect(env.CODYAI_WINDOWS_TASK_NAME).toBe("CodyAI Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.CODYAI_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through CODYAI_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CODYAI_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.CODYAI_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to CODYAI_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.CODYAI_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers CODYAI_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        CODYAI_GATEWAY_TOKEN: "openclaw-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.CODYAI_GATEWAY_TOKEN).toBe("openclaw-token");
  });

  it("omits CODYAI_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", CODYAI_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", CODYAI_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw"));
  });

  it("uses CODYAI_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", CODYAI_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in CODYAI_STATE_DIR", () => {
    const env = { HOME: "/Users/test", CODYAI_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { CODYAI_STATE_DIR: "C:\\State\\openclaw" };
