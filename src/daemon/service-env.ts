...
export function buildServiceEnvironment(params: {
  ...
  return {
    ...
    CODYAI_PROFILE: profile,
    CODYAI_GATEWAY_PORT: String(port),
    CODYAI_LAUNCHD_LABEL: resolvedLaunchdLabel,
    CODYAI_SYSTEMD_UNIT: systemdUnit,
    CODYAI_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    CODYAI_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    CODYAI_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.CODYAI_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    CODYAI_GATEWAY_TOKEN: gatewayToken,
    CODYAI_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    CODYAI_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    CODYAI_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    CODYAI_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    CODYAI_LOG_PREFIX: "node",
    CODYAI_SERVICE_MARKER: NODE_SERVICE_MARKER,
    CODYAI_SERVICE_KIND: NODE_SERVICE_KIND,
    CODYAI_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    CODYAI_STATE_DIR: sharedEnv.stateDir,
    CODYAI_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.CODYAI_STATE_DIR;
  const configPath = env.CODYAI_CONFIG_PATH;
