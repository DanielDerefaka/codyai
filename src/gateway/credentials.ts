  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set CODYAI_GATEWAY_TOKEN/CODYAI_GATEWAY_PASSWORD, pass explicit --token/--password,",
 ...
 * This prevents literal placeholder strings like `${CODYAI_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.CODYAI_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.CODYAI_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
export function resolveGatewayCredentialsFromConfig(params: {
  ...
  const localTokenPrecedence =
    ...
    (env.CODYAI_SERVICE_KIND === "gateway" ? "config-first" : "env-first");
