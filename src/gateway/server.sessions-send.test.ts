...
const { createCodyAITools } = await import("../agents/openclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createCodyAITools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createCodyAITools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["CODYAI_GATEWAY_PORT", "CODYAI_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.CODYAI_GATEWAY_PORT = String(gatewayPort);
  process.env.CODYAI_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("CODYAI_CONFIG_PATH missing in gateway test environment");
      }
