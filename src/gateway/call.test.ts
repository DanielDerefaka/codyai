...
describe("callGateway url resolution", () => {
  const envSnapshot = captureEnv([
    "CODYAI_ALLOW_INSECURE_PRIVATE_WS",
    "CODYAI_GATEWAY_URL",
  ...
  it("uses CODYAI_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    loadConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.CODYAI_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.CODYAI_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env CODYAI_GATEWAY_URL when set", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.CODYAI_GATEWAY_URL;
    try {
      process.env.CODYAI_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env CODYAI_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.CODYAI_GATEWAY_URL;
      } else {
        process.env.CODYAI_GATEWAY_URL = prevUrl;
      }
    }
  });
  ...
  it("allows ws:// private remote URLs only when CODYAI_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.CODYAI_ALLOW_INSECURE_PRIVATE_WS = "1";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
  ...
  it("allows ws:// hostname remote URLs when CODYAI_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.CODYAI_ALLOW_INSECURE_PRIVATE_WS = "1";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.CODYAI_GATEWAY_TOKEN = "env-token";
    process.env.CODYAI_GATEWAY_PASSWORD = "env-password";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.CODYAI_GATEWAY_URL = "wss://override.example/ws";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "CODYAI_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.CODYAI_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.CODYAI_GATEWAY_PASSWORD = "from-env";
    loadConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
