...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "CODYAI_STATE_DIR",
  "CODYAI_CONFIG_PATH",
  "CODYAI_SKIP_BROWSER_CONTROL_SERVER",
  "CODYAI_SKIP_GMAIL_WATCHER",
  "CODYAI_SKIP_CANVAS_HOST",
  "CODYAI_BUNDLED_PLUGINS_DIR",
  "CODYAI_SKIP_CHANNELS",
  "CODYAI_SKIP_PROVIDERS",
  "CODYAI_SKIP_CRON",
  "CODYAI_TEST_MINIMAL_GATEWAY",
] as const;
...
function applyGatewaySkipEnv() {
  process.env.CODYAI_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.CODYAI_SKIP_GMAIL_WATCHER = "1";
  process.env.CODYAI_SKIP_CANVAS_HOST = "1";
  process.env.CODYAI_SKIP_CHANNELS = "1";
  process.env.CODYAI_SKIP_PROVIDERS = "1";
  process.env.CODYAI_SKIP_CRON = "1";
  process.env.CODYAI_TEST_MINIMAL_GATEWAY = "1";
  process.env.CODYAI_BUNDLED_PLUGINS_DIR = tempHome
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["CODYAI_GATEWAY_TOKEN"]);
  const prev = process.env.CODYAI_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CODYAI_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.CODYAI_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
