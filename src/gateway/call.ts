...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.CODYAI_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.CODYAI_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set CODYAI_ALLOW_INSECURE_PRIVATE_WS=1",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__CODYAI_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
