...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", CODYAI_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
      const cfg: CodyAIConfig = {
        channels: {
          telegram: { accounts: { work: { botToken: "tok-work" } } },
        },
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): CodyAIConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): CodyAIConfig => ({
    channels: {
      telegram: {
