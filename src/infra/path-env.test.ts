...
let ensureCodyAICliOnPath: typeof import("./path-env.js").ensureCodyAICliOnPath;

describe("ensureCodyAICliOnPath", () => {
  const envKeys = [
    "PATH",
    "CODYAI_PATH_BOOTSTRAPPED",
    "CODYAI_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureCodyAICliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openclaw exists", () => {
    ...
    const cliPath = path.join(appBinDir, "codyai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.CODYAI_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.CODYAI_PATH_BOOTSTRAPPED = "1";
    ensureCodyAICliOnPath({
  ...
  it("prepends mise shims when available", () => {
    ...
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "codyai");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    ...
    const localCli = path.join(localBinDir, "codyai");
