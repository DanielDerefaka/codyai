...
vi.mock("./openclaw-root.js", () => ({
  resolveCodyAIPackageRoot: vi.fn(async () => null),
  resolveCodyAIPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveCodyAIPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openclaw");
    (
      openclawRoot.resolveCodyAIPackageRoot as unknown as ReturnType<typeof vi.fn>
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "codyai" }));
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    (
      openclawRoot.resolveCodyAIPackageRootSync as unknown as ReturnType<typeof vi.fn>
