...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.CODYAI_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.CODYAI_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set CODYAI_APNS_TEAM_ID and CODYAI_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.CODYAI_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.CODYAI_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.CODYAI_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set CODYAI_APNS_PRIVATE_KEY_P8 or CODYAI_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading CODYAI_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createCodyAIPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    openclaw: createCodyAIPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
