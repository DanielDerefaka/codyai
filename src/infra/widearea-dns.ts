...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.CODYAI_WIDE_AREA_DOMAIN ?? null;
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "codyai";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "codyai");
