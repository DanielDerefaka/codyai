...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CODYAI_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.CODYAI_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveCodyAIPackageRoot } = await import("./openclaw-root.js"));
    ...
    vi.mocked(resolveCodyAIPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveCodyAIPackageRoot).mockResolvedValue("/opt/openclaw");
