...
const WINDOWS_TASK_SUPERVISOR_HINT_ENV_VARS = ["CODYAI_WINDOWS_TASK_NAME"] as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.CODYAI_SERVICE_MARKER?.trim();
    const serviceKind = env.CODYAI_SERVICE_KIND?.trim();
