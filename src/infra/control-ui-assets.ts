...
import { resolveCodyAIPackageRoot, resolveCodyAIPackageRootSync } from "./openclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveCodyAIPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "codyai" + dist/control-ui/index.html
