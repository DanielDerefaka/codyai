...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: CodyAIConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: CodyAIConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: CodyAIConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: CodyAIConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: CodyAIConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: CodyAIConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: CodyAIConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
