...
function candidateBinDirs(opts: EnsureCodyAIPathOpts): { prepend: string[]; append: string[] } {
  ...
  try {
    ...
    const siblingCli = path.join(execDir, "codyai");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.CODYAI_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "codyai"))) {
 ...
 */
export function ensureCodyAICliOnPath(opts: EnsureCodyAIPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CODYAI_PATH_BOOTSTRAPPED)) {
    return;
  }
