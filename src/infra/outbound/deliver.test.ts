...
import { resolvePreferredCodyAITmpDir } from "../tmp-openclaw-dir.js";
...
const telegramChunkConfig: CodyAIConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: CodyAIConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes CodyAI tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredCodyAITmpDir()]),
      }),
    );
  });

  it("includes CodyAI tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes CodyAI tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes CodyAI tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: CodyAIConfig = { channels: { signal: { mediaMaxMb: 2 } } };
