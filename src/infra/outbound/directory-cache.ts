...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: CodyAIConfig | null = null;
  ...
  get(key: string, cfg: CodyAIConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: CodyAIConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: CodyAIConfig): void {
