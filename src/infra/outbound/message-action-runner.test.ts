...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred CodyAI tmp root", async () => {
    const tmpRoot = resolvePreferredCodyAITmpDir();
    ...
    try {
      ...
      const hostTmpOutsideCodyAI = path.join(os.tmpdir(), "outside-openclaw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
