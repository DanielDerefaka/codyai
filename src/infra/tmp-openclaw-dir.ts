...
export const POSIX_CODYAI_TMP_DIR = "/tmp/openclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredCodyAITmpDirOptions = {
...
export function resolvePreferredCodyAITmpDir(
  options: ResolvePreferredCodyAITmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "codyai" : `openclaw-${uid}`;
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback CodyAI temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback CodyAI temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_CODYAI_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_CODYAI_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_CODYAI_TMP_DIR)) {
      return POSIX_CODYAI_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_CODYAI_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_CODYAI_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_CODYAI_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_CODYAI_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_CODYAI_TMP_DIR;
