...
describe("resolveEffectiveHomeDir", () => {
  it("prefers CODYAI_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands CODYAI_HOME when set to ~", () => {
    const env = {
      CODYAI_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for CODYAI_HOME", () => {
    const result = resolveRequiredHomeDir(
      { CODYAI_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when CODYAI_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ CODYAI_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { CODYAI_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
    });
