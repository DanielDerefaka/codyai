...
export function triggerCodyAIRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.CODYAI_SYSTEMD_UNIT,
      process.env.CODYAI_PROFILE,
    );
  ...
  const label =
    process.env.CODYAI_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CODYAI_PROFILE);
