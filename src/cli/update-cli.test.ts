...
import type { CodyAIConfig, ConfigFileSnapshot } from "../config/types.openclaw.js";
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveCodyAIPackageRoot: vi.fn(),
}));
...
const { resolveCodyAIPackageRoot } = await import("../infra/openclaw-root.js");
...
describe("update-cli", () => {
  ...
  const baseConfig = {} as CodyAIConfig;
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveCodyAIPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveCodyAIPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("codyai");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("CodyAI update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as CodyAIConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveCodyAIPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ CODYAI_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.CODYAI_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    await withEnvAsync({ CODYAI_GIT_DIR: tempDir }, async () => {
      setTty(true);
