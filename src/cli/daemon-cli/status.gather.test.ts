...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    CODYAI_STATE_DIR: "/tmp/openclaw-daemon",
    CODYAI_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.CODYAI_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.CODYAI_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODYAI_STATE_DIR = "/tmp/openclaw-cli";
    process.env.CODYAI_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.CODYAI_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
