...
beforeEach(() => {
  ...
  originalHideBanner = process.env.CODYAI_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.CODYAI_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.CODYAI_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("codyai");
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "codyai", "status", "--debug"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "codyai", "message", "send"],
    });
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "codyai", "--version"],
    });
    ...
    vi.clearAllMocks();
    process.env.CODYAI_HIDE_BANNER = "1";

    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "codyai", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "codyai", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "codyai", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "codyai", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "codyai", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "codyai", "backup", "create", "--json"],
    });
