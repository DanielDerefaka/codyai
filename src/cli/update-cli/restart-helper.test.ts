...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("uses CODYAI_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        CODYAI_PROFILE: "default",
        CODYAI_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("uses CODYAI_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        CODYAI_PROFILE: "default",
        CODYAI_LAUNCHD_LABEL: "com.custom.openclaw",
      });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CodyAI Gateway"');
      expect(content).toContain('schtasks /Run /TN "CodyAI Gateway"');
    ...
    it("uses CODYAI_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        CODYAI_PROFILE: "default",
        CODYAI_WINDOWS_TASK_NAME: "CodyAI Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "CodyAI Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "CodyAI Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "CodyAI Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        CODYAI_LAUNCHD_LABEL: "ai.openclaw.it's-a-test",
      });
