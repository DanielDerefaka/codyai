...
import { buildWebSearchProviderConfig, withTempHome, writeCodyAIConfig } from "./test-helpers.js";
import { CodyAISchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = CodyAISchema.safeParse({
  ...
  it("rejects non-string $schema", () => {
    const result = CodyAISchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = CodyAISchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeCodyAIConfig(home, {
        gateway: { bind: "${CODYAI_BIND}" },
      });

      const prev = process.env.CODYAI_BIND;
      process.env.CODYAI_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.CODYAI_BIND;
        } else {
          process.env.CODYAI_BIND = prev;
        }
      }
    });
  });
