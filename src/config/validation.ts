...
import type { CodyAIConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: CodyAIConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: CodyAIConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: CodyAIConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = CodyAISchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as CodyAIConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as CodyAIConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as CodyAIConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as CodyAIConfig,
  };
}
