...
function collectConfigEnvVarsByTarget(cfg?: CodyAIConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: CodyAIConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: CodyAIConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: CodyAIConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.CODYAI_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
