...
describe("oauth paths", () => {
  it("prefers CODYAI_OAUTH_DIR over CODYAI_STATE_DIR", () => {
    const env = {
      CODYAI_OAUTH_DIR: "/custom/oauth",
  ...
  it("derives oauth path from CODYAI_STATE_DIR when unset", () => {
    const env = {
      CODYAI_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  ...
  function expectCodyAIHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.CODYAI_HOME;
    if (!configuredHome) {
      throw new Error("CODYAI_HOME must be set for this assertion helper");
    }
  ...
  it("uses CODYAI_STATE_DIR when set", () => {
    const env = {
      CODYAI_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;

    expect(resolveStateDir(env, () => "/home/test")).toBe(path.resolve("/new/state"));
  });

  it("uses CODYAI_HOME for default state/config locations", () => {
    const env = {
      CODYAI_HOME: "/srv/openclaw-home",
    } as NodeJS.ProcessEnv;
    expectCodyAIHomeDefaults(env);
  });

  it("prefers CODYAI_HOME over HOME for default state/config locations", () => {
    const env = {
      CODYAI_HOME: "/srv/openclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv;
  ...
  it("respects state dir overrides when config is missing", async () => {
    await withTempRoot("openclaw-config-override-", async (root) => {
      ...
      const env = { CODYAI_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
