...
function isWhatsAppConfigured(cfg: CodyAIConfig): boolean {
...
function isGenericChannelConfigured(cfg: CodyAIConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: CodyAIConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: CodyAIConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: CodyAIConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: CodyAIConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: CodyAIConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: CodyAIConfig, pluginId: string): CodyAIConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
