...
const ETC_CODYAI_DIR = path.join(ROOT_DIR, "etc", "codyai");
...
function etcCodyAIPath(...parts: string[]) {
  return path.join(ETC_CODYAI_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcCodyAIPath("agents.json");
