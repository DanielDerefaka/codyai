...
import type { CodyAIConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareCodyAIVersions } from "./version.js";
...
function unsetPathForWrite(
  root: CodyAIConfig,
  pathSegments: string[],
): { changed: boolean; next: CodyAIConfig } {
...
function coerceConfig(value: unknown): CodyAIConfig {
...
function stampConfigVersion(cfg: CodyAIConfig): CodyAIConfig {
...
function warnIfConfigFromFuture(cfg: CodyAIConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareCodyAIVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer CodyAI (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as CodyAIConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): CodyAIConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as CodyAIConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: CodyAIConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as CodyAIConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.CODYAI_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.CODYAI_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.CODYAI_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.CODYAI_WATCH_SESSION === "string" &&
        deps.env.CODYAI_WATCH_SESSION.trim().length > 0
          ? deps.env.CODYAI_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.CODYAI_WATCH_COMMAND === "string" &&
        deps.env.CODYAI_WATCH_COMMAND.trim().length > 0
          ? deps.env.CODYAI_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `CODYAI_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: CodyAIConfig | null = null;
let runtimeConfigSourceSnapshot: CodyAIConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.CODYAI_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.CODYAI_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): CodyAIConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): CodyAIConfig | null {
  return runtimeConfigSourceSnapshot;
}

export function loadConfig(): CodyAIConfig {
...
export async function readBestEffortConfig(): Promise<CodyAIConfig> {
