...
export type ConfigSchema = ReturnType<typeof CodyAISchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = CodyAISchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "CodyAIConfig";
  const hints = applyDerivedTags(mapSensitivePaths(CodyAISchema, "", buildBaseHints()));
  const next = {
