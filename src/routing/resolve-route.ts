...
function listAgents(cfg: CodyAIConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: CodyAIConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<CodyAIConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: CodyAIConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: CodyAIConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: CodyAIConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<CodyAIConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  CodyAIConfig,
  {
    bindingsRef: CodyAIConfig["bindings"];
    agentsRef: CodyAIConfig["agents"];
    sessionRef: CodyAIConfig["session"];
...
function resolveRouteCacheForConfig(cfg: CodyAIConfig): Map<string, ResolvedAgentRoute> {
