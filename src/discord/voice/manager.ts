...
function resolveVoiceTtsConfig(params: { cfg: CodyAIConfig; override?: TtsConfig }): {
  cfg: CodyAIConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredCodyAITmpDir(), "discord-voice-"));
