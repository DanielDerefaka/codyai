...
describe("shared/frontmatter", () => {
  ...
  test("resolveCodyAIManifestBlock parses JSON5 metadata and picks openclaw block", () => {
    const frontmatter = {
      metadata: "{ openclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveCodyAIManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveCodyAIManifestBlock returns undefined for invalid input", () => {
    expect(resolveCodyAIManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveCodyAIManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveCodyAIManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
