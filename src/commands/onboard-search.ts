...
function rawKeyValue(config: CodyAIConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: CodyAIConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: CodyAIConfig, provider: SearchProvider): CodyAIConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: CodyAIConfig, result: CodyAIConfig): CodyAIConfig {
...
export async function setupSearch(
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — CodyAI will store a reference instead of the API key.",
