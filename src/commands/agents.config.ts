...
function resolveAgentName(cfg: CodyAIConfig, agentId: string) {
...
function resolveAgentModel(cfg: CodyAIConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: CodyAIConfig): AgentSummary[] {
