...
describe("readServiceStatusSummary", () => {
  it("marks CodyAI-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["codyai", "gateway", "run"] })),
    ...
    expect(summary.managedByCodyAI).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByCodyAI).toBe(false);
