...
export function collectMissingDefaultAccountBindingWarnings(cfg: CodyAIConfig): string[] {
...
export function collectMissingExplicitDefaultAccountWarnings(cfg: CodyAIConfig): string[] {
  const warnings: string[] = [];
...
function scanTelegramAllowFromUsernameEntries(cfg: CodyAIConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: CodyAIConfig): Promise<{
  config: CodyAIConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: CodyAIConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: CodyAIConfig): {
  config: CodyAIConfig;
  changes: string[];
} {
...
function scanMutableAllowlistEntries(cfg: CodyAIConfig): MutableAllowlistHit[] {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: CodyAIConfig): {
  config: CodyAIConfig;
  changes: string[];
} {
...
async function maybeRepairAllowlistPolicyAllowFrom(cfg: CodyAIConfig): Promise<{
  config: CodyAIConfig;
  changes: string[];
}> {
  const channels = cfg.channels;
 ...
 * file support.
 */
function detectEmptyAllowlistPolicy(cfg: CodyAIConfig): string[] {
  const channels = cfg.channels;
  if (!channels || typeof channels !== "object") {
    return [];
  }
...
function collectExecSafeBinScopes(cfg: CodyAIConfig): ExecSafeBinScopeRef[] {
...
function scanExecSafeBinCoverage(cfg: CodyAIConfig): ExecSafeBinCoverageHit[] {
...
function scanExecSafeBinTrustedDirHints(cfg: CodyAIConfig): ExecSafeBinTrustedDirHintHit[] {
...
function maybeRepairExecSafeBinProfiles(cfg: CodyAIConfig): {
  config: CodyAIConfig;
  changes: string[];
  warnings: string[];
} {
...
function scanLegacyToolsBySenderKeys(cfg: CodyAIConfig): LegacyToolsBySenderKeyHit[] {
  const hits: LegacyToolsBySenderKeyHit[] = [];
  collectLegacyToolsBySenderKeyHits(cfg, [], hits);
  return hits;
}

function maybeRepairLegacyToolsBySenderKeys(cfg: CodyAIConfig): {
  config: CodyAIConfig;
  changes: string[];
} {
  const next = structuredClone(cfg);
