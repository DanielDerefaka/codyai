...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    pluginRegistry.list = [];
    prevToken = process.env.CODYAI_GATEWAY_TOKEN;
    prevPassword = process.env.CODYAI_GATEWAY_PASSWORD;
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as CodyAIConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.CODYAI_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as CodyAIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("treats SecretRef token config as authenticated for exposure warning level", async () => {
    const cfg = {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: { source: "env", provider: "default", id: "CODYAI_GATEWAY_TOKEN" },
        },
      },
    } as CodyAIConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as CodyAIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as CodyAIConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
