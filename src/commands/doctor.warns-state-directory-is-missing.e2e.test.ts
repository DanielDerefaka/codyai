...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    process.env.CODYAI_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when CODYAI_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.CODYAI_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
