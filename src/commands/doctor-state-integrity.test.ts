...
type EnvSnapshot = {
  HOME?: string;
  CODYAI_HOME?: string;
  CODYAI_STATE_DIR?: string;
  CODYAI_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    CODYAI_HOME: process.env.CODYAI_HOME,
    CODYAI_STATE_DIR: process.env.CODYAI_STATE_DIR,
    CODYAI_OAUTH_DIR: process.env.CODYAI_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: CodyAIConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: CodyAIConfig) {
...
async function runStateIntegrityText(cfg: CodyAIConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    process.env.CODYAI_HOME = tempHome;
    process.env.CODYAI_STATE_DIR = path.join(tempHome, ".openclaw");
    delete process.env.CODYAI_OAUTH_DIR;
    fs.mkdirSync(process.env.CODYAI_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when CODYAI_OAUTH_DIR is explicitly configured", async () => {
    process.env.CODYAI_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: CodyAIConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
