...
import { CodyAISchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: CodyAIConfig): {
  config: CodyAIConfig;
  removed: string[];
} {
  const parsed = CodyAISchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: CodyAIConfig): void {
