...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.CODYAI_GATEWAY_TOKEN ?? process.env.CLAWDBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "CodyAI update wizard" : "CodyAI configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: CodyAIConfig = snapshot.valid ? snapshot.config : {};
