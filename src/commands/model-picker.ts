...
type PromptDefaultModelResult = { model?: string; config?: CodyAIConfig };
...
function resolveConfiguredModelRaw(cfg: CodyAIConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: CodyAIConfig): string[] {
...
export function applyPrimaryModel(cfg: CodyAIConfig, model: string): CodyAIConfig {
...
export function applyModelAllowlist(cfg: CodyAIConfig, models: string[]): CodyAIConfig {
  const defaults = cfg.agents?.defaults;
