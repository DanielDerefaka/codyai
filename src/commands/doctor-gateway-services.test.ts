...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.CODYAI_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.CODYAI_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
async function runRepair(cfg: CodyAIConfig) {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      CODYAI_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service CODYAI_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: CodyAIConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.CODYAI_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses CODYAI_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ CODYAI_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            CODYAI_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing CodyAI gateway next.",
    );
  });
});
