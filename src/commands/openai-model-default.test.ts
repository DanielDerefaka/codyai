...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: CodyAIConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: CodyAIConfig) => { changed: boolean; next: CodyAIConfig };
  defaultModel: string;
  overrideConfig: CodyAIConfig;
  alreadyDefaultConfig: CodyAIConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: CodyAIConfig) => config,
      applyDefaultConfig: (config: CodyAIConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: CodyAIConfig = { agents: { defaults: {} } };
