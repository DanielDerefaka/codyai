...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for CODYAI_STATE_DIR.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // CODYAI_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: CodyAIConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.CODYAI_OAUTH_DIR?.trim()) {
    return true;
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: CODYAI_STATE_DIR=~/.openclaw ${formatCliCommand("openclaw doctor")}`,
      ].join("\n"),
    );
  }
