...
export const resolveCodyAIPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadCodyAIPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveCodyAIPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.CODYAI_STATE_DIR;
  originalUpdateInProgress = process.env.CODYAI_UPDATE_IN_PROGRESS;
  process.env.CODYAI_UPDATE_IN_PROGRESS = "1";
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.CODYAI_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
