...
export function buildConfigureCandidates(config: CodyAIConfig): ConfigureCandidate[] {
...
export function buildConfigureCandidatesForScope(params: {
  config: CodyAIConfig;
  authoredCodyAIConfig?: CodyAIConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredCodyAIConfig ?? params.config;
