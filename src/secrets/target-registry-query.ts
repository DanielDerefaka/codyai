...
const CODYAI_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of CODYAI_COMPILED_SECRET_TARGETS) {
...
const CODYAI_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: CODYAI_COMPILED_SECRET_TARGETS,
    entriesById: CODYAI_TARGETS_BY_ID,
  });
