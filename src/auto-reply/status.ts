...
type AgentDefaults = NonNullable<NonNullable<CodyAIConfig["agents"]>["defaults"]>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 CodyAI ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: CodyAIConfig): string {
  const lines = ["ℹ️ Help", ""];
