...
type HomeEnvSnapshot = {
  ...
  CODYAI_STATE_DIR: string | undefined;
  CODYAI_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    CODYAI_AGENT_DIR: process.env.CODYAI_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.CODYAI_STATE_DIR = path.join(home, ".openclaw");
    process.env.CODYAI_AGENT_DIR = path.join(home, ".openclaw", "agent");
