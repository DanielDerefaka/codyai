...
import { scheduleGatewaySigusr1Restart, triggerCodyAIRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting CodyAI in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerCodyAIRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting CodyAI via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
