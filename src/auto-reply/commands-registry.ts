...
export function isCommandEnabled(cfg: CodyAIConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: CodyAIConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as CodyAIConfig),
...
export function getCommandDetection(_cfg?: CodyAIConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: CodyAIConfig) {
