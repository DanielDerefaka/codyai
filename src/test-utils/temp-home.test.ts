...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.CODYAI_STATE_DIR;
    ...
    expect(process.env.CODYAI_STATE_DIR).toBe(path.join(tempHome.home, ".openclaw"));
    ...
    expect(process.env.CODYAI_STATE_DIR).toBe(previousStateDir);
