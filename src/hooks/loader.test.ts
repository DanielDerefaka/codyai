...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["CODYAI_BUNDLED_HOOKS_DIR"]);
    process.env.CODYAI_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  describe("loadInternalHooks", () => {
    ...
    const expectNoCommandHookRegistration = async (cfg: CodyAIConfig) => {
    ...
    it("rejects directory hook handlers that escape hook dir via symlink", async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'metadata: {"codyai":{"events":["command:new"]}}',
          "---",
          "",
          "# Symlink Hook",
        ].join("\n"),
