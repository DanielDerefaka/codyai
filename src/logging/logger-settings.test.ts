...
let originalCodyAILogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.CODYAI_TEST_FILE_LOG;
  originalCodyAILogLevel = process.env.CODYAI_LOG_LEVEL;
  delete process.env.CODYAI_TEST_FILE_LOG;
  delete process.env.CODYAI_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.CODYAI_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalCodyAILogLevel === undefined) {
    delete process.env.CODYAI_LOG_LEVEL;
  } else {
    process.env.CODYAI_LOG_LEVEL = originalCodyAILogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.CODYAI_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
