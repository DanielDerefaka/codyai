...
import type { CodyAIConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredCodyAITmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "openclaw.log"); // legacy single-file path

const LOG_PREFIX = "codyai";
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: CodyAIConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg && !shouldSkipLoadConfigFallback()) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => CodyAIConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.CODYAI_TEST_FILE_LOG !== "1" ? "silent" : "info";
