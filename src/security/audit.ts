...
import type { ConfigFileSnapshot, CodyAIConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: CodyAIConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your CodyAI state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your CodyAI state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.CODYAI_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.CODYAI_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.CODYAI_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: CodyAIConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: CodyAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: CodyAIConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set CODYAI_GATEWAY_TOKEN/CODYAI_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
