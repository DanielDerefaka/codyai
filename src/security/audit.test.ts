...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: CodyAIConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: CodyAIConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: CodyAIConfig) => string[];
  isConfigured?: (account: unknown, cfg: CodyAIConfig) => boolean;
  isEnabled?: (account: unknown, cfg: CodyAIConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ CODYAI_STATE_DIR: sharedChannelSecurityStateDir }, () =>
...
`,
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.CODYAI_GATEWAY_TOKEN;
    const prevPassword = process.env.CODYAI_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.CODYAI_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.CODYAI_GATEWAY_PASSWORD;
      } else {
        process.env.CODYAI_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: CodyAIConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: CodyAIConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): CodyAIConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: CodyAIConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: CodyAIConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: CodyAIConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: CodyAIConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.CODYAI_GATEWAY_TOKEN;
    process.env.CODYAI_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: CodyAIConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<CodyAIConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: CodyAIConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: CodyAIConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.CODYAI_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.CODYAI_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
