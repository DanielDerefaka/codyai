...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set CODYAI_GATEWAY_TOKEN/CODYAI_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.CODYAI_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.CODYAI_GATEWAY_PASSWORD);
