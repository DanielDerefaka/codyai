...
// Prefix for CodyAI-generated action IDs to scope our handler
const CODYAI_ACTION_PREFIX = "openclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${CODYAI_ACTION_PREFIX}`);

  // Handle CodyAI modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
