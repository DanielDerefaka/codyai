...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        CODYAI_BASE_ENV: "base",
        CODYAI_TO_REMOVE: undefined,
      },
      env: {
        CODYAI_TEST_ENV: "ok",
      },
    ...
    expect(resolved.CODYAI_BASE_ENV).toBe("base");
    expect(resolved.CODYAI_TEST_ENV).toBe("ok");
    expect(resolved.CODYAI_TO_REMOVE).toBeUndefined();
  });
