...
import type { GatewayBindMode, GatewayTailscaleMode, CodyAIConfig } from "../config/config.js";
...
type ConfigureGatewayOptions = {
  flow: WizardFlow;
  baseConfig: CodyAIConfig;
  nextConfig: CodyAIConfig;
  localPort: number;
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForOnboarding({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "CODYAI_GATEWAY_TOKEN",
          copy: {
            sourceMessage: "Where is this gateway token stored?",
            envVarPlaceholder: "CODYAI_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ?? normalizeGatewayTokenInput(process.env.CODYAI_GATEWAY_TOKEN)) ||
        randomToken();
      ...
      const tokenInput = await prompter.text({
        ...
        initialValue:
          ...
          normalizeGatewayTokenInput(process.env.CODYAI_GATEWAY_TOKEN) ??
          "",
      });
    ...
    if (!password) {
      const selectedMode = await resolveSecretInputModeForEnvSelection({
        ...
        copy: {
          ...
          plaintextHint: "Stores the password directly in CodyAI config",
        },
      });
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForOnboarding({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "CODYAI_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: "Where is this gateway password stored?",
            envVarPlaceholder: "CODYAI_GATEWAY_PASSWORD",
          },
        });
