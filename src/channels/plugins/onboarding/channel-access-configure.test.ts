...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: CodyAIConfig, policy: ChannelAccessPolicy) => CodyAIConfig;
  resolveAllowlist: (params: { cfg: CodyAIConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: CodyAIConfig; resolved: TResolved }) => CodyAIConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: CodyAIConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: CodyAIConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: CodyAIConfig, policy: ChannelAccessPolicy): CodyAIConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: CodyAIConfig, policy: ChannelAccessPolicy): CodyAIConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: CodyAIConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: CodyAIConfig; resolved: string[] }) => {
      calls.push("apply");
