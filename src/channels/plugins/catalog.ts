...
import { discoverCodyAIPlugins } from "../../plugins/discovery.js";
import type { CodyAIPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, CodyAIPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["CODYAI_PLUGIN_CATALOG_PATHS", "CODYAI_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<CodyAIPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: CodyAIPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: CodyAIPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverCodyAIPlugins({ workspaceDir: options.workspaceDir });
