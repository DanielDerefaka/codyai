...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  ...
  const cliPath = path.join(root, "codyai");
...
resolve_openclaw_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_CODYAI_BIN: cliPath,
        CODYAI_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("CodyAI 2026.3.8 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("CodyAI dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("CodyAI dev's build");
    },
  );
