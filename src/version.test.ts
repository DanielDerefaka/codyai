...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "codyai", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "codyai", version: "2.3.4" });
  ...
  it("prefers CODYAI_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        CODYAI_VERSION: "9.9.9",
        CODYAI_SERVICE_VERSION: "2.2.2",
