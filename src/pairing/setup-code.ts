...
function resolveGatewayTokenFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return env.CODYAI_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
}

function resolveGatewayPasswordFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    env.CODYAI_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim() || undefined
  );
}

function resolveAuth(cfg: CodyAIConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
...
async function resolveGatewayTokenSecretRef(
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.CODYAI_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
    );
