...
declare const __CODYAI_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "codyai";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["CODYAI_VERSION"],
      runtimeVersion,
      env["CODYAI_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current CodyAI version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __CODYAI_VERSION__ === "string" ? __CODYAI_VERSION__ : undefined,
  bundledVersion: process.env.CODYAI_BUNDLED_VERSION,
});
