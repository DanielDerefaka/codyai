...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): CodyAIConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as CodyAIConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): CodyAIConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as CodyAIConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: CodyAIConfig, accountId = "media") {
    await handleTelegramAction(
