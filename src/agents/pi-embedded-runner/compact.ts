...
import { resolveCodyAIDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveCodyAIAgentDir();
  await ensureCodyAIModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createCodyAICodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveCodyAIDocsPath({
      workspaceDir: effectiveWorkspace,
