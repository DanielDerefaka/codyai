...
import { createCodyAICodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    try {
      ...
      const tools = createCodyAICodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: CodyAIConfig) {
    return createCodyAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<CodyAIConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<CodyAIConfig["agents"]>["list"]>[number]["tools"];
  }): CodyAIConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createCodyAICodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createCodyAICodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createCodyAICodingTools({
      config: cfg,
    ...
    const defaultTools = createCodyAICodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createCodyAICodingTools({
      config: cfg,
    ...
    const bobTools = createCodyAICodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createCodyAICodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createCodyAICodingTools({
      config: cfg,
