...
type AgentEntry = NonNullable<NonNullable<CodyAIConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: CodyAIConfig): AgentEntry[] {
...
export function listAgentIds(cfg: CodyAIConfig): string[] {
...
export function resolveDefaultAgentId(cfg: CodyAIConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: CodyAIConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: CodyAIConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: CodyAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: CodyAIConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
