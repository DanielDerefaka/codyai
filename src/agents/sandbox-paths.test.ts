...
async function withOutsideHardlinkInCodyAITmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked CodyAI tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked CodyAI tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCodyAITmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked CodyAI tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInCodyAITmp(
