...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    previousFastTestEnv = process.env.CODYAI_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.CODYAI_TEST_FAST;
      return;
    }
    process.env.CODYAI_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // CODYAI_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("CodyAI runtime context (internal):");
