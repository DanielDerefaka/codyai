...
describe("resolveCodyAIAgentDir", () => {
  ...
  it("defaults to the multi-agent path when no overrides are set", async () => {
    await withTempStateDir((stateDir) => {
      withEnv(
        ...
        () => {
          const resolved = resolveCodyAIAgentDir();
  ...
  it("honors CODYAI_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          CODYAI_STATE_DIR: undefined,
          CODYAI_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveCodyAIAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when CODYAI_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers CODYAI_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          CODYAI_STATE_DIR: undefined,
          CODYAI_AGENT_DIR: primaryOverride,
