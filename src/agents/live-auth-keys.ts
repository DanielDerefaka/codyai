...
const GOOGLE_LIVE_SINGLE_KEY = "CODYAI_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "CODYAI_LIVE_ANTHROPIC_KEY",
    listVar: "CODYAI_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "CODYAI_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `CODYAI_LIVE_${base}_KEY`;
