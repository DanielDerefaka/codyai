...
const defaultTools = createCodyAICodingTools({ senderIsOwner: true });
...
describe("createCodyAICodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: CodyAIConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createCodyAICodingTools({
    ...
    const anthropicTools = createCodyAICodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: CodyAIConfig = {
      tools: {
        exec: {
    ...
    const allowed = createCodyAICodingTools({
    ...
    const denied = createCodyAICodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createCodyAICodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
