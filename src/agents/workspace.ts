...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.CODYAI_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".codyai", `workspace-${profile}`);
  }
  return path.join(home, ".codyai", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".codyai";
