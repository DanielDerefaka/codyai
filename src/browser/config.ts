import type { BrowserConfig, BrowserProfileConfig, CodyAIConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_CODYAI_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_CODYAI_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "codyai" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_CODYAI_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_CODYAI_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an CodyAI browser profile (routing config), not a Chrome user profile.
...
export function resolveBrowserConfig(
  ...
  rootConfig?: CodyAIConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_CODYAI_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_CODYAI_BROWSER_PROFILE_NAME]
        ? DEFAULT_CODYAI_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "codyai";
