...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredCodyAITmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredCodyAITmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredCodyAITmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredCodyAITmpDir).toHaveBeenCalled();
  });
