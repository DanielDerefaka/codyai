...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.CODYAI_GATEWAY_PORT = prevGatewayPort;
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchCodyAIChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveCodyAIUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopCodyAIChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.CODYAI_GATEWAY_PORT;
  process.env.CODYAI_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.CODYAI_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.CODYAI_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.CODYAI_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.CODYAI_GATEWAY_PASSWORD;
  } else {
    process.env.CODYAI_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
