...
function localCodyAIProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "codyai",
...
function createLocalCodyAIResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localCodyAIProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveCodyAIUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    ...
    const profileDir = path.join(tempRoot, "codyai");
    ...
    const ops = createLocalCodyAIResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveCodyAIUserDataDir: () => profileDir,
    });
