...
let tempDirIndex = 0;
const prevBundledDir = process.env.CODYAI_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.CODYAI_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadCodyAIPlugins({
      cache: false,
  ...
  process.env.CODYAI_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.CODYAI_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadCodyAIPlugins>) {
...
function useNoBundledPlugins() {
  process.env.CODYAI_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadCodyAIPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.CODYAI_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.CODYAI_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadCodyAIPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadCodyAIPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadCodyAIPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadCodyAIPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ CODYAI_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
  ...
  register() {},
    ...
    const script = `
      import { loadCodyAIPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadCodyAIPlugins({
        cache: false,
