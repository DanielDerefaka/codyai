...
import type {
  CodyAIPluginApi,
  CodyAIPluginChannelRegistration,
  CodyAIPluginCliRegistrar,
  CodyAIPluginCommandDefinition,
  CodyAIPluginHttpRouteAuth,
  CodyAIPluginHttpRouteMatch,
  CodyAIPluginHttpRouteHandler,
  CodyAIPluginHttpRouteParams,
  CodyAIPluginHookOptions,
  ProviderPlugin,
  CodyAIPluginService,
  CodyAIPluginToolContext,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: CodyAIPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: CodyAIPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: CodyAIPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: CodyAIPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: CodyAIPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: CodyAIPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: CodyAIPluginHookOptions | undefined,
    config: CodyAIPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: CodyAIPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: CodyAIPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as CodyAIPluginChannelRegistration).plugin === "object"
        ? (registration as CodyAIPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: CodyAIPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: CodyAIPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: CodyAIPluginCommandDefinition) => {
