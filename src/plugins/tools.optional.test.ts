...
const loadCodyAIPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadCodyAIPlugins: (params: unknown) => loadCodyAIPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadCodyAIPluginsMock.mockReturnValue(registry);
  return registry;
}
...
describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadCodyAIPluginsMock.mockClear();
  });
