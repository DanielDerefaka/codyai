...
INSTALL_URL="${CODYAI_INSTALL_URL:-https://openclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${CODYAI_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CODYAI_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openclaw"
PACKAGE_NAME="${CODYAI_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CODYAI_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CODYAI_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${CODYAI_INSTALL_LATEST_OUT:-}"
fi
