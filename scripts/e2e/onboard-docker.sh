...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    CODYAI_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    CODYAI_ENTRY="dist/index.js"
	  else
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$CODYAI_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$CODYAI_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }
  ...
  set_isolated_openclaw_env() {
    ...
    export CODYAI_HOME="$home_dir"
    export CODYAI_STATE_DIR="$home_dir/.openclaw"
    export CODYAI_CONFIG_PATH="$CODYAI_STATE_DIR/openclaw.json"
    mkdir -p "$CODYAI_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$CODYAI_STATE_DIR/workspace"
    config_path="$CODYAI_CONFIG_PATH"
    sessions_dir="$CODYAI_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_openclaw_env "$home_dir"
	    ...
	    node "$CODYAI_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$CODYAI_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$CODYAI_ENTRY\" configure --section skills" send_skills_flow
