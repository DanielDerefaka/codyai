...
WAIT_FOR_LOCK=0
LOG_PATH="${CODYAI_RESTART_LOG:-/tmp/openclaw-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${CODYAI_GATEWAY_WAIT_SECONDS:-0}"
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  CODYAI_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
...
choose_app_bundle() {
  ...
  fail "App bundle not found. Set CODYAI_APP_BUNDLE to your installed OpenClaw.app"
}
