...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openclawHome: process.env.CODYAI_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CODYAI_HOME", snapshot.openclawHome);
  restoreKey("CODYAI_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked CODYAI_HOME.
  delete process.env.CODYAI_HOME;
  process.env.CODYAI_STATE_DIR = path.join(base, ".openclaw");
