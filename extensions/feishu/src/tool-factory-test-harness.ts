import type { AnyAgentTool, CodyAIPluginApi } from "openclaw/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: CodyAIPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<CodyAIPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
