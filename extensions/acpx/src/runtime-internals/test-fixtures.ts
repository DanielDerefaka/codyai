...
import { resolvePreferredCodyAITmpDir } from "../../../../src/infra/tmp-openclaw-dir.js";
...
const openclawShell = process.env.CODYAI_SHELL || "";
...
async function ensureMockCliScriptPath(): Promise<string> {
  ...
  sharedMockCliScriptPath = (async () => {
    const dir = await mkdtemp(
      path.join(resolvePreferredCodyAITmpDir(), "openclaw-acpx-runtime-test-"),
    );
    tempDirs.push(dir);
