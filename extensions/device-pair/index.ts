...
import type { CodyAIPluginApi } from "openclaw/plugin-sdk/device-pair";
import {
...
function resolveGatewayPort(cfg: CodyAIPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.CODYAI_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: CodyAIPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: CodyAIPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the CodyAI iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the CodyAI iOS app:",
            "",
            "```",
            qrAscii,
            "```",
