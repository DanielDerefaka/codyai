...
import type { CodyAIPluginApi, CodyAIPluginToolContext } from "openclaw/plugin-sdk/lobster";
...
function fakeApi(overrides: Partial<CodyAIPluginApi> = {}): CodyAIPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<CodyAIPluginToolContext> = {}): CodyAIPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: CodyAIPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
