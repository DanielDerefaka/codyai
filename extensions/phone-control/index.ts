...
import type { CodyAIPluginApi, CodyAIPluginService } from "openclaw/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: CodyAIPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: CodyAIPluginApi["config"]): string[] {
...
export default function register(api: CodyAIPluginApi) {
  ...
  const timerService: CodyAIPluginService = {
