---
summary: "VPS hosting hub for CodyAI (Oracle/Fly/Hetzner/GCP/exe.dev)"
read_when:
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/codyai-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `CODYAI_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for stable startup path:
  - `CODYAI_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/codyai-compile-cache`
