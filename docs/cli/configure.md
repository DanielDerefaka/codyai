---
summary: "CLI reference for `codyai configure` (interactive configuration prompts)"
read_when:
...
Tip: `codyai config` without a subcommand opens the same wizard. Use
`codyai config get|set|unset` for non-interactive edits.
...
```bash
codyai configure
codyai configure --section model --section channels
```
