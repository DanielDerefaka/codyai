---
summary: "CLI reference for `codyai onboard` (interactive onboarding wizard)"
read_when:
...
```bash
codyai onboard
codyai onboard --flow quickstart
codyai onboard --flow manual
codyai onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`CODYAI_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `codyai dashboard` (Control UI, no channel setup).
