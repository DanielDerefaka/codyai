...
The onboarding wizard is the **recommended** way to set up CodyAI on macOS,
...
`codyai dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`codyai configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.codyai/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `codyai doctor` first.
</Note>
...
Use `codyai agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.codyai/workspace-<agentId>`.
...
- CLI command reference: [`codyai onboard`](/cli/onboard)
