...
有关调试"为什么这被阻止了？"，请参见[沙箱 vs 工具策略 vs 提权](/gateway/sandbox-vs-tool-policy-vs-elevated) 和 `codyai sandbox explain`。
...
- `group:nodes`：`nodes`
- `group:codyai`：所有内置 CodyAI 工具（不包括提供商插件）
...
旧版 `agent.*` 配置由 `codyai doctor` 迁移；今后请优先使用 `agents.defaults` + `agents.list`。
