---
read_when:
  - 你想在 CodyAI 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 CodyAI 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
codyai plugins enable minimax-portal-auth  # 如果已加载则跳过
codyai gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/codyai/codyai/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `codyai models list` 和 `codyai models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `codyai configure` 并选择 **MiniMax M2.1**，或
