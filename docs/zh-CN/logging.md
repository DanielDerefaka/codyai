...
你可以在 `~/.codyai/codyai.json` 中覆盖此设置：
...
所有日志配置都在 `~/.codyai/codyai.json` 的 `logging` 下。
...
- CodyAI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `codyai plugins enable diagnostics-otel` 启用插件。
...
- `codyai.tokens`（计数器，属性：`codyai.token`、`codyai.channel`、`codyai.provider`、`codyai.model`）
- `codyai.cost.usd`（计数器，属性：`codyai.channel`、`codyai.provider`、`codyai.model`）
- `codyai.run.duration_ms`（直方图，属性：`codyai.channel`、`codyai.provider`、`codyai.model`）
- `codyai.context.tokens`（直方图，属性：`codyai.context`、`codyai.channel`、`codyai.provider`、`codyai.model`）

消息流：

- `codyai.webhook.received`（计数器，属性：`codyai.channel`、`codyai.webhook`）
- `codyai.webhook.error`（计数器，属性：`codyai.channel`、`codyai.webhook`）
- `codyai.webhook.duration_ms`（直方图，属性：`codyai.channel`、`codyai.webhook`）
- `codyai.message.queued`（计数器，属性：`codyai.channel`、`codyai.source`）
- `codyai.message.processed`（计数器，属性：`codyai.channel`、`codyai.outcome`）
- `codyai.message.duration_ms`（直方图，属性：`codyai.channel`、`codyai.outcome`）

队列 + 会话：

- `codyai.queue.lane.enqueue`（计数器，属性：`codyai.lane`）
- `codyai.queue.lane.dequeue`（计数器，属性：`codyai.lane`）
- `codyai.queue.depth`（直方图，属性：`codyai.lane` 或 `codyai.channel=heartbeat`）
- `codyai.queue.wait_ms`（直方图，属性：`codyai.lane`）
- `codyai.session.state`（计数器，属性：`codyai.state`、`codyai.reason`）
- `codyai.session.stuck`（计数器，属性：`codyai.state`）
- `codyai.session.stuck_age_ms`（直方图，属性：`codyai.state`）
- `codyai.run.attempt`（计数器，属性：`codyai.attempt`）
...
- `codyai.webhook.error`
  - `codyai.channel`、`codyai.webhook`、`codyai.chatId`、`codyai.error`
- `codyai.message.processed`
  - `codyai.channel`、`codyai.outcome`、`codyai.chatId`、`codyai.messageId`、`codyai.sessionKey`、`codyai.sessionId`、`codyai.reason`
- `codyai.session.stuck`
  - `codyai.state`、`codyai.ageMs`、`codyai.queueDepth`、`codyai.sessionKey`、`codyai.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `codyai doctor`。
