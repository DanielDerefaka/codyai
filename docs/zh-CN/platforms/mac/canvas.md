...
- `~/Library/Application Support/CodyAI/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `codyai-canvas://<session>/<path>`

示例：

- `codyai-canvas://main/` → `<canvasRoot>/main/index.html`
- `codyai-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `codyai-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
codyai nodes canvas present --node <id>
codyai nodes canvas navigate --node <id> --url "/"
codyai nodes canvas eval --node <id> --js "document.title"
codyai nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__codyai__/a2ui/
```
...
codyai nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
codyai nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "codyai://agent?message=Review%20this%20design";
```
