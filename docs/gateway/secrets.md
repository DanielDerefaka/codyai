...
CodyAI supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `CODYAI_GATEWAY_TOKEN` (or `CLAWDBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, CodyAI runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). CodyAI validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/CodyAI QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/codyai"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `codyai.json` refs)
...
When reload-time activation fails after a healthy state, CodyAI enters degraded secrets state.
...
- Strict command paths (for example `codyai memory` remote-memory paths and `codyai qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `codyai status`, `codyai status --all`, `codyai channels status`, `codyai channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `codyai secrets reload`.
...
```bash
codyai secrets audit --check
codyai secrets configure
codyai secrets audit --check
```
...
- plaintext values at rest (`codyai.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `codyai.json` refs)
...
- lets you select supported secret-bearing fields in `codyai.json` plus `auth-profiles.json` for one agent scope
...
- `codyai secrets configure --providers-only`
- `codyai secrets configure --skip-provider-setup`
- `codyai secrets configure --agent <id>`
...
```bash
codyai secrets apply --from /tmp/codyai-secrets-plan.json
codyai secrets apply --from /tmp/codyai-secrets-plan.json --dry-run
```
...
CodyAI intentionally does not write rollback backups containing historical plaintext secret values.
