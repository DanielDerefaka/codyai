...
`codyai doctor` is the repair + migration tool for CodyAI. It fixes stale
...
```bash
codyai doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
codyai doctor --repair --force
```
...
- Extra workspace dir detection (`~/codyai`).
...
you to run `codyai doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.codyai/codyai.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.codyai/sessions/` to `~/.codyai/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.codyai/agent/` to `~/.codyai/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.codyai/credentials/*.json` (except `oauth.json`)
  - to `~/.codyai/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `codyai doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.codyai` folders exist across
  home directories or when `CODYAI_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.codyai/codyai.json` is
...
offers to remove them and install the CodyAI service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named CodyAI gateway services are considered first-class and are not
flagged as "extra."
...
- `codyai doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `codyai doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
- `codyai doctor` prompts before rewriting supervisor config.
- `codyai doctor --yes` accepts the default repair prompts.
- `codyai doctor --repair` applies recommended fixes without prompts.
- `codyai doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `codyai gateway install --force`.
