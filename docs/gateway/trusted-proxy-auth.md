...
read_when:
  - Running CodyAI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of CodyAI
...
- You run CodyAI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. CodyAI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. CodyAI extracts the user identity from the configured header
...
- CodyAI can stay on loopback HTTP behind the proxy.
...
If CodyAI itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://codyai.example.com
    to: http://codyai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`codyai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update CodyAI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `codyai security audit` and review findings
