...
CodyAI supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `codyai gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.codyai/.env` so the daemon can read it:
...
API keys for daemon use: `codyai onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.codyai/.env`, systemd/launchd).
...
```bash
codyai models auth paste-token --provider anthropic
codyai models auth paste-token --provider openrouter
```
...
- Priority order:
  - `CODYAI_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- CodyAI retries with the next key only for rate-limit errors (for example
...
Run `codyai models status` to confirm which profile is expiring. If the profile
