...
- During heartbeat runs, CodyAI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `codyai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, CodyAI skips the heartbeat run entirely (no model call).
...
```bash
codyai system event --text "Check for urgent follow-ups" --mode now
```
