---
summary: "Use OpenAI via API keys or Codex subscription in CodyAI"
read_when:
  - You want to use OpenAI models in CodyAI
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like CodyAI.
...
```bash
codyai onboard --auth-choice openai-api-key
# or non-interactive
codyai onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. CodyAI forwards both through the `openai/*` Responses path.
...
codyai onboard --auth-choice openai-codex

# Or run OAuth directly
codyai models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. CodyAI
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

CodyAI uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), CodyAI also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. CodyAI enables it by default for
...
CodyAI, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), CodyAI now auto-enables OpenAI server-side
