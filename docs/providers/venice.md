---
summary: "Use Venice AI privacy-focused models in CodyAI"
read_when:
  - You want privacy-focused inference in CodyAI
...
```bash
codyai onboard --auth-choice venice-api-key
```
...
```bash
codyai agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, CodyAI shows all available Venice models. Pick based on your needs:
...
```bash
codyai models set venice/kimi-k2-5
codyai models set venice/claude-opus-4-6
```

List all available models:

```bash
codyai models list | grep venice
```

## Configure via `codyai configure`

1. Run `codyai configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
CodyAI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
codyai agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
codyai agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
codyai agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
codyai agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
codyai agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `codyai models list` to see currently available models. Some models may be temporarily offline.
