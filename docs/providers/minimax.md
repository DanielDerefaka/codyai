---
summary: "Use MiniMax M2.5 in CodyAI"
read_when:
  - You want MiniMax models in CodyAI
...
```bash
codyai plugins enable minimax-portal-auth  # skip if already loaded.
codyai gateway restart  # restart if gateway is already running
codyai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/codyai/codyai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `codyai.json`:
...
- Use `codyai models list` and `codyai models set minimax/MiniMax-M2.5` to switch.
...
- Running `codyai configure` and selecting **MiniMax M2.5**, or
