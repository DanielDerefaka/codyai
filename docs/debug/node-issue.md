...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in CodyAI
...
Running CodyAI via Node with `tsx` fails at startup with:

```
[codyai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `codyai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch codyai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node codyai.mjs status` works on Node 25.
