...
read_when:
  - You want to place an outbound voice call from CodyAI
...
Voice calls for CodyAI via a plugin. Supports outbound notifications and
...
- Use `codyai voicecall ...` or the `voice_call` tool
...
```bash
codyai plugins install ./extensions/voice-call
...
```bash
codyai voicecall call --to "+15555550123" --message "Hello from CodyAI"
codyai voicecall continue --call-id <id> --message "Any questions?"
codyai voicecall speak --call-id <id> --message "One moment"
codyai voicecall end --call-id <id>
codyai voicecall status --call-id <id>
codyai voicecall tail
