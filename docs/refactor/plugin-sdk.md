...
- Publish as `codyai/plugin-sdk` (or export from core under `codyai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `CodyAIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: CodyAIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: CodyAIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: CodyAIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: CodyAIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: CodyAIConfig): string;
  };
};
```
...
- Introduce `codyai/plugin-sdk`.
- Add `api.runtime` to `CodyAIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `codyaiRuntime: ">=2026.2.0"`).
