...
Deploy CodyAI on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy CodyAI](https://northflank.com/stacks/deploy-codyai) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy CodyAI now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the CodyAI template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the CodyAI service.
8. Open the public CodyAI URL and complete setup at `/setup`.
9. Open the Control UI at `/codyai`.

## What you get

- Hosted CodyAI Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/codyai`
