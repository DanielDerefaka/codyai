...
Deploy CodyAI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/codyai/codyai)
...
repository configures everything needed to run CodyAI:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CODYAI_STATE_DIR
        value: /data/.codyai
      - key: CODYAI_WORKSPACE_DIR
        value: /data/workspace
      - key: CODYAI_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/codyai`.
...
If you use the original CodyAI repository, Render will not auto-deploy your CodyAI. To update it, run a manual Blueprint sync from the dashboard.
...
For CodyAI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any CodyAI host.
