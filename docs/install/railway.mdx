...
Deploy CodyAI on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/codyai` — Control UI
...
- `CODYAI_STATE_DIR=/data/.codyai` (recommended)
- `CODYAI_WORKSPACE_DIR=/data/workspace` (recommended)
- `CODYAI_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your CodyAI state + workspace so you can migrate to another host without losing config or memory.
