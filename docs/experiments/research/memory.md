...
read_when:
  - Designing workspace memory (~/.codyai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep CodyAI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.codyai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.codyai/workspace` as canonical human-readable memory.
...
```
~/.codyai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in CodyAI**, but keep a separable core library.

### Why integrate into CodyAI?

- CodyAI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `codyai memory recall "…" --k 25 --since 30d`
  - `codyai memory reflect --since 7d`
...
Pragmatic take for `~/.codyai/workspace`:
