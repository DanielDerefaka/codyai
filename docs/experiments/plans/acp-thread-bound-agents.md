...
This plan defines how CodyAI should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- CodyAI core owns ACP control plane concerns
...
CodyAI should not reimplement ACP transport internals in core.
CodyAI should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in CodyAI, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under CodyAI state dir
...
- ACP event stream is normalized to CodyAI reply chunks
...
For the first production backend (`extensions/acpx`), CodyAI and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses CodyAI `sessionKey`)
...
- CodyAI consumes ndjson events from `acpx --format json --json-strict`
