...
- With Anthropic API-key auth profiles, CodyAI seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, CodyAI injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
CodyAI exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.codyai/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$CODYAI_STATE_DIR/logs/cache-trace.jsonl`
...
- `CODYAI_CACHE_TRACE=1` enables cache tracing.
- `CODYAI_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `CODYAI_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `CODYAI_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `CODYAI_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
