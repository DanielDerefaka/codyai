...
This is the full reference for the `codyai onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.codyai/credentials/oauth.json`; auth profiles live in `~/.codyai/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.codyai/credentials/oauth.json` (or `$CODYAI_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.codyai/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `codyai pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `codyai configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `codyai health`.
    - Tip: `codyai status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export CODYAI_GATEWAY_TOKEN="your-token"
codyai onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env CODYAI_GATEWAY_TOKEN
```
...
- Stores it under `~/.codyai/tools/signal-cli/<version>/`.
...
Typical fields in `~/.codyai/codyai.json`:
...
`codyai agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.codyai/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.codyai/agents/<agentId>/sessions/`.
