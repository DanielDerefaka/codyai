...
CodyAI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- CodyAI owns lifecycle when `channels.signal.autoStart=true`.
...
- CodyAI spawns `imsg rpc` as a child process (legacy iMessage integration).
