...
CodyAI features that can generate provider usage or paid API calls.
...
- `codyai status --usage` and `codyai channels list` show provider **usage windows**
...
CodyAI can pick up credentials from:
...
`codyai models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
