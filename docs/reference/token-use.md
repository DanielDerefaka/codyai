---
summary: "How CodyAI builds prompt context and reports token usage + costs"
read_when:
...
CodyAI tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

CodyAI assembles its own system prompt on every run. It includes:
...
For images, CodyAI downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from CodyAI session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `codyai status --usage` and `codyai channels list` show
...
`cacheWrite`. If pricing is missing, CodyAI shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. CodyAI can
...
Anthropic's 1M context window is currently beta-gated. CodyAI can inject the
...
CodyAI skips the `context-1m-*` beta header because Anthropic currently
