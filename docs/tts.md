...
CodyAI can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere CodyAI can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, CodyAI defaults
...
TTS config lives under `messages.tts` in `codyai.json`.
...
- If `provider` is **unset**, CodyAI prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.codyai/settings/tts.json`, override with `CODYAI_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, CodyAI retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so CodyAI registers
