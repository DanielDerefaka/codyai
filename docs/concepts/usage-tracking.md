...
- `/usage cost` in chats: local cost summary aggregated from CodyAI session logs.
- CLI: `codyai status --usage` prints a full per-provider breakdown.
- CLI: `codyai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
