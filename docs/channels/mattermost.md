---
summary: "Mattermost bot setup and CodyAI config"
read_when:
...
```bash
codyai plugins install @codyai/mattermost
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/mattermost
```
...
4. Configure CodyAI and start the gateway.
...
Native slash commands are opt-in. When enabled, CodyAI registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, CodyAI derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as CodyAI.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to CodyAI.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from CodyAI, not `404`.
...
- Approve via:
  - `codyai pairing list mattermost`
  - `codyai pairing approve mattermost <CODE>`
...
Use these target formats with `codyai message send` or cron/webhooks:
...
- If `interactions.callbackBaseUrl` is omitted, CodyAI derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and CodyAI run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="codyai-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"codyai-mattermost-interactions",
...
`codyai message send` and cron/webhook deliveries.
