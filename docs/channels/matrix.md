...
Matrix is an open, decentralized messaging protocol. CodyAI connects as a Matrix **user**
...
```bash
codyai plugins install @codyai/matrix
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `codyai plugins install @codyai/matrix`
   - From a local checkout: `codyai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: CodyAI calls the same
     login endpoint, stores the access token in `~/.codyai/credentials/matrix/credentials.json`,
...
- On first connection, CodyAI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  CodyAI logs a warning.
...
`~/.codyai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `codyai pairing list matrix`
  - `codyai pairing approve matrix <CODE>`
...
- On startup, CodyAI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
