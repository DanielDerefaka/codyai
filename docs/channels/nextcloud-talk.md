...
```bash
codyai plugins install @codyai/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "CodyAI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `codyai pairing list nextcloud-talk`
  - `codyai pairing approve nextcloud-talk <CODE>`
