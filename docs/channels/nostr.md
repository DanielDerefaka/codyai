...
read_when:
  - You want CodyAI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables CodyAI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`codyai onboard`) and `codyai channels add` list optional channel plugins.
...
```bash
codyai plugins install @codyai/nostr
```

Use a local checkout (dev workflows):

```bash
codyai plugins install --link <path-to-codyai>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "codyai@example.com",
        "lud16": "codyai@example.com"
      }
    }
  }
}
