...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects CodyAI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current CodyAI releases, so no separate plugin install
is required.
...
```bash
codyai plugins install @codyai/feishu
```
...
If you just installed CodyAI, run the wizard:
...
2. Configuring app credentials in CodyAI
...
1. You already ran `codyai channels add` for Feishu
2. The gateway is running (`codyai gateway status`)
...
```bash
codyai pairing approve feishu <CODE>
```
...
2. Run `codyai logs --follow` and look for `chat_id`
...
2. Run `codyai logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `codyai gateway status`  | Show gateway status           |
| `codyai gateway install` | Install/start gateway service |
| `codyai gateway stop`    | Stop gateway service          |
| `codyai gateway restart` | Restart gateway service       |
| `codyai logs --follow`   | Tail gateway logs             |
...
4. Check logs: `codyai logs --follow`
...
5. Ensure the gateway is running: `codyai gateway status`
6. Check logs: `codyai logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.codyai/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.codyai/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
