...
read_when:
  - Setting up Zalo Personal for CodyAI
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside CodyAI.
...
- Install via CLI: `codyai plugins install @codyai/zalouser`
- Or from a source checkout: `codyai plugins install ./extensions/zalouser`
...
```bash
codyai directory self --channel zalouser
codyai directory peers list --channel zalouser --query "name"
codyai directory groups list --channel zalouser --query "work"
```
...
- `codyai pairing list zalouser`
- `codyai pairing approve zalouser <code>`
...
- On startup, CodyAI resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, CodyAI stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in CodyAI state. Example:
...
- CodyAI sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, CodyAI sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `codyai channels logout --channel zalouser && codyai channels login --channel zalouser`
...
- The channel now runs fully in CodyAI without external CLI binaries.
