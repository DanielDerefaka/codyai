...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure CodyAI HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `codyai pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while CodyAI is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
CodyAI supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, CodyAI falls back to normal delivery for remaining payloads.
