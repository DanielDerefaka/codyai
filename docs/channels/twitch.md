...
read_when:
  - Setting up Twitch chat integration for CodyAI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. CodyAI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
codyai plugins install @codyai/twitch
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `CODYAI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "codyai", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use CODYAI_TWITCH_ACCESS_TOKEN env var)
...
```bash
CODYAI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
