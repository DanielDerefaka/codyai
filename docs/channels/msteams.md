...
```bash
codyai plugins install @codyai/msteams
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
CodyAI will offer the local install path automatically.
...
3. Configure CodyAI with those credentials.
...
- Talk to CodyAI via Teams DMs, group chats, or channels.
...
- On startup, CodyAI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.codyai/codyai.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring CodyAI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `codyai-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `codyai plugins install @codyai/msteams`
   - From a local checkout: `codyai plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "CodyAI in Teams", "full": "CodyAI in Teams" },
...
CodyAI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, CodyAI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/CodyAIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

CodyAI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `codyai message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.codyai/msteams-polls.json`.
...
```bash
codyai message send --channel msteams \
...
codyai message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
codyai message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
codyai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
codyai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
