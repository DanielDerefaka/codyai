...
Tlon is a decentralized messenger built on Urbit. CodyAI connects to your Urbit ship and can
...
```bash
codyai plugins install @codyai/tlon
```

Local checkout (when running from a git repo):

```bash
codyai plugins install ./extensions/tlon
```
...
By default, CodyAI blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `codyai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, CodyAI replies in-thread.
