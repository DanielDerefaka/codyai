...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `codyai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `codyai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=codyai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${CODYAI_STATE_DIR:-$HOME/.codyai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
