...
- **gateway host** → `codyai` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.codyai/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.codyai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `codyai approvals allowlist ...`).
- `codyai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `codyai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.codyai/exec-approvals.json` directly.

CLI: `codyai approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
