---
summary: "CodyAI plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends CodyAI with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core CodyAI yet (or you want to keep optional features out of your main
install).
...
```bash
codyai plugins install @codyai/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@codyai/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@codyai/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@codyai/zalouser`
- [Matrix](/channels/matrix) — `@codyai/matrix`
- [Nostr](/channels/nostr) — `@codyai/nostr`
- [Zalo](/channels/zalo) — `@codyai/zalo`
- [Microsoft Teams](/channels/msteams) — `@codyai/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

CodyAI plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `codyai/plugin-sdk` import when
authoring plugins:

- `codyai/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `codyai/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `codyai/plugin-sdk/telegram` for Telegram channel plugins.
- `codyai/plugin-sdk/discord` for Discord channel plugins.
- `codyai/plugin-sdk/slack` for Slack channel plugins.
- `codyai/plugin-sdk/signal` for Signal channel plugins.
- `codyai/plugin-sdk/imessage` for iMessage channel plugins.
- `codyai/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `codyai/plugin-sdk/line` for LINE channel plugins.
- `codyai/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `codyai/plugin-sdk/acpx`, `codyai/plugin-sdk/bluebubbles`,
  `codyai/plugin-sdk/copilot-proxy`, `codyai/plugin-sdk/device-pair`,
  `codyai/plugin-sdk/diagnostics-otel`, `codyai/plugin-sdk/diffs`,
  `codyai/plugin-sdk/feishu`,
  `codyai/plugin-sdk/google-gemini-cli-auth`, `codyai/plugin-sdk/googlechat`,
  `codyai/plugin-sdk/irc`, `codyai/plugin-sdk/llm-task`,
  `codyai/plugin-sdk/lobster`, `codyai/plugin-sdk/matrix`,
  `codyai/plugin-sdk/mattermost`, `codyai/plugin-sdk/memory-core`,
  `codyai/plugin-sdk/memory-lancedb`,
  `codyai/plugin-sdk/minimax-portal-auth`,
  `codyai/plugin-sdk/nextcloud-talk`, `codyai/plugin-sdk/nostr`,
  `codyai/plugin-sdk/open-prose`, `codyai/plugin-sdk/phone-control`,
  `codyai/plugin-sdk/qwen-portal-auth`, `codyai/plugin-sdk/synology-chat`,
  `codyai/plugin-sdk/talk-voice`, `codyai/plugin-sdk/test-utils`,
  `codyai/plugin-sdk/thread-ownership`, `codyai/plugin-sdk/tlon`,
  `codyai/plugin-sdk/twitch`, `codyai/plugin-sdk/voice-call`,
  `codyai/plugin-sdk/zalo`, and `codyai/plugin-sdk/zalouser`.

Compatibility note:

- `codyai/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `codyai status`, `codyai status --all`,
  `codyai channels status`, `codyai channels resolve`, and doctor/config
...
- Set `CODYAI_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `CODYAI_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `CODYAI_PLUGIN_DISCOVERY_CACHE_MS` and
  `CODYAI_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

CodyAI scans, in order:
...
- `<workspace>/.codyai/extensions/*.ts`
- `<workspace>/.codyai/extensions/*/index.ts`

3. Global extensions

- `~/.codyai/extensions/*.ts`
- `~/.codyai/extensions/*/index.ts`

4. Bundled extensions (shipped with CodyAI, mostly disabled by default)

- `<codyai>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `codyai plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, CodyAI logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. CodyAI blocks candidates when:
...
Each plugin must include a `codyai.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `codyai.extensions`:
...
Security guardrail: every `codyai.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `codyai plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `codyai.channel` and
install hints via `codyai.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@codyai/nextcloud-talk",
  "codyai": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
CodyAI can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.codyai/mpm/plugins.json`
- `~/.codyai/mpm/catalog.json`
- `~/.codyai/plugins/catalog.json`

Or point `CODYAI_PLUGIN_CATALOG_PATHS` (or `CODYAI_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "codyai": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, CodyAI uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `codyai.plugin.json` (`configSchema`).
...
CodyAI augments `uiHints` at runtime based on discovered plugins:
...
```bash
codyai plugins list
codyai plugins info <id>
codyai plugins install <path>                 # copy a local file/dir into ~/.codyai/extensions/<id>
codyai plugins install ./extensions/voice-call # relative path ok
codyai plugins install ./plugin.tgz           # install from a local tarball
codyai plugins install ./plugin.zip           # install from a local zip
codyai plugins install -l ./extensions/voice-call # link (no copy) for dev
codyai plugins install @codyai/voice-call # install from npm
codyai plugins install @codyai/voice-call --pin # store exact resolved name@version
codyai plugins update <id>
codyai plugins update --all
codyai plugins enable <id>
codyai plugins disable <id>
codyai plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, CodyAI warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `codyai voicecall`).
...
- Plugin-managed hooks show up in `codyai hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `codyai hooks`; enable/disable the plugin instead.
...
- When disabled, CodyAI blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside CodyAI (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `codyai models auth login --provider <id> [--method <id>]`
...
- `config`: The current CodyAI config
...
- Main package: `codyai` (this repo)
- Plugins: separate npm packages under `@codyai/*` (example: `@codyai/voice-call`)

Publishing contract:

- Plugin `package.json` must include `codyai.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `codyai plugins install <npm-spec>` uses `npm pack`, extracts into `~/.codyai/extensions/<id>/`, and enables it in config.
...
- CLI: `codyai voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `codyai.extensions` points at the built entrypoint (`dist/index.js`).
