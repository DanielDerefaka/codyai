---
summary: "Chrome extension: let CodyAI drive your existing Chrome tab"
read_when:
...
The CodyAI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate codyai-managed Chrome profile.
...
CodyAI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the CodyAI release (npm package) as static files. There is no separate “build” step.

After upgrading CodyAI:

- Re-run `codyai browser extension install` to refresh the installed files under your CodyAI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

CodyAI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `CODYAI_GATEWAY_TOKEN`)
...
- Open the tab you want CodyAI to control.
...
- `ON`: attached; CodyAI can drive that tab.
...
Debugging: `codyai sandbox explain`
...
`codyai browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `codyai browser extension install` first to copy the extension to a stable location under your CodyAI state directory.
...
- **This is not isolated** like the dedicated codyai-managed profile.
