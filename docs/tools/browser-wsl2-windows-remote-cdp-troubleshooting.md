...
read_when:
  - Running CodyAI Gateway in WSL2 while Chrome lives on Windows
...
- CodyAI Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/codyai/codyai/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the CodyAI Chrome extension.
...
- CodyAI points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, CodyAI is not the problem yet.
...
Fix that before touching CodyAI config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point CodyAI at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting CodyAI to succeed
...
```bash
codyai browser open https://example.com --browser-profile remote
codyai browser tabs --browser-profile remote
```

For the extension relay:

```bash
codyai browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `codyai browser tabs` returns the target
...
3. CodyAI config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug CodyAI config or Control UI auth
