...
- Same path: run the app from a fixed location (for CodyAI, `dist/CodyAI.app`).
...
```bash
sudo tccutil reset Accessibility ai.codyai.mac
sudo tccutil reset ScreenCapture ai.codyai.mac
...
Workaround: move files into the CodyAI workspace (`~/.codyai/workspace`) if you want to avoid per-folder grants.
