...
service via the external `codyai` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.codyai.gateway`
  (or `ai.codyai.<profile>` when using `--profile`/`CODYAI_PROFILE`; legacy `com.codyai.*` is supported).
