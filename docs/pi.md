...
summary: "Architecture of CodyAI's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in CodyAI
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how CodyAI integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

CodyAI uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, CodyAI directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in CodyAI's local TUI mode)                                             |
...
├── pi-tools.ts                    # createCodyAICodingTools()
...
├── codyai-tools.ts              # CodyAI-specific tools
...
Image injection is prompt-local: CodyAI loads image refs from the current prompt and
...
2. **Custom Replacements**: CodyAI replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **CodyAI Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures CodyAI's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, CodyAI CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
CodyAI wraps this with `guardSessionManager()` for tool result safety.
...
CodyAI maintains an auth profile store with multiple API keys per provider:
...
CodyAI loads custom pi extensions for specialized behavior:
...
CodyAI also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | CodyAI Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom CodyAI tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.codyai/agents/<agentId>/sessions/` (or `$CODYAI_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `CODYAI_LIVE_TEST=1`)
