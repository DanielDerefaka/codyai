...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/codyai`)
...
If the page fails to load, start the Gateway first: `codyai gateway`.
...
you revoke it with `codyai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.codyai/codyai.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop codyai`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. CodyAI
...
CodyAI **blocks** Control UI connections without device identity.
