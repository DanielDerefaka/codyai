...
```bash
codyai tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `codyai tui --deliver`
...
- Local shell commands receive `CODYAI_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `codyai logs --follow`.
- Confirm the agent can run: `codyai status` and `codyai models status`.
...
- No agents in picker: check `codyai agents list` and your routing config.
