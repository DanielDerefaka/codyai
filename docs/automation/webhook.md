...
read_when:
  ...
  - Wiring external systems into CodyAI
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your CodyAI config directory (typically `~/.codyai/hooks/transforms`).
...
- `codyai webhooks gmail setup` writes `hooks.gmail` config for `codyai webhooks gmail run`.
