...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in CodyAI.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in CodyAI. See [Webhook Hooks](/automation/webhook) or use `codyai webhooks` for Gmail helper commands.
...
- Extend CodyAI's behavior without modifying core code

## Getting Started

### Bundled Hooks

CodyAI ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.codyai/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.codyai/logs/commands.log`
...
During onboarding (`codyai onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.codyai/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<codyai>/dist/hooks/bundled/` (shipped with CodyAI)
...
Hook packs are standard npm packages that export one or more hooks via `codyai.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.codyai/hooks/<id>`.
Each `codyai.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `codyai hooks install` installs dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.codyai.ai/automation/hooks#my-hook
metadata:
  { "codyai": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.codyai` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before CodyAI persists them.
...
- **Managed hooks** (`~/.codyai/hooks/`): Shared across workspaces

### 2. Create Directory Structure

```bash
mkdir -p ~/.codyai/hooks/my-hook
cd ~/.codyai/hooks/my-hook
```
...
metadata: { "codyai": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# JSON output
codyai hooks info session-memory --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.codyai/workspace`)
...
```yaml
metadata: { "codyai": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "codyai": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.codyai/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.codyai/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.codyai/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/codyai/codyai/tree/main/src/hooks/bundled)
