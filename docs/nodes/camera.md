...
CodyAI supports **camera capture** for agent workflows:
...
```bash
codyai nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
codyai nodes camera snap --node <id> --facing front
codyai nodes camera clip --node <id> --duration 3000
codyai nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`codyai.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `codyai` CLI to invoke camera commands on the macOS node.

Examples:

```bash
codyai nodes camera list --node <id>            # list camera ids
codyai nodes camera snap --node <id>            # prints MEDIA:<path>
codyai nodes camera snap --node <id> --max-width 1280
codyai nodes camera snap --node <id> --delay-ms 2000
codyai nodes camera snap --node <id> --device-id <id>
codyai nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
codyai nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
codyai nodes camera clip --node <id> --device-id <id>
codyai nodes camera clip --node <id> --no-audio
```

Notes:

- `codyai nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
codyai nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
